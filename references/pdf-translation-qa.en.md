...
- Dark text becomes invisible on a dark cell: the contrast fallback mainly handles white-on-light text. If you hit dark-on-dark, add an exact glossary entry or post-edit by hand.
...
- Company names (for example `Apple Inc.`, `Goldman Sachs`) appear in English unless the document's convention says otherwise.
- Tickers and exchange codes (for example `AAPL`, `NYSE:MSFT`, `005930.KS`) are untouched.
- Product/model names (for example `iPhone 15`, `Model Y`, `A100`, `GPT-4`) are untouched.
...
When you find a table cell that should have stayed English: add the source string to the glossary's `keep_as_source` list, delete its entry from the cache or set value=source, and rerun the rebuild step. Do **not** rely on `--no-auto-preserve`; that just disables the heuristic globally.

## Editing Postprocessing

`postprocess_translation` in the script applies a few generic cleanups such as old machine-translation artifacts. Do **not** add unconditional `replace()` calls for real Chinese words there; they will silently corrupt valid translations across future PDFs.

Document-specific bad terms belong in the glossary's `bad_terms` list for warnings or `replacements` list for substitutions.

## Suggested Comparison Pages

Render at least:

- The cover or opening page.
- One or two dense text pages.
- Two dense table pages if present.
- One chart or figure page if present.
- The first and last appendix, legal, disclaimer, or disclosure pages.
...
Search the generated translation cache and output text for obvious artifacts. If these appear, fix the cache and rerun the renderer:
